...
def test_summary_method(report, ls):
    for col in ls.columns:
        ...
        assert ((summary['desc'] == 'categorical') ==
                report['column_properties'][col]['is_categorical'])

        assert ((summary['desc'] == 'numeric') ==
                (report['column_properties'][col]['numeric'] and
                 not report['column_properties'][col]['is_categorical']))


def test_numeric_details(report, ls):
    num_cols = [col for col in ls.columns
...
def test_histogram(report, ls):
    ...
    for col in num_cols:
        ...
        for key, actual in zip(['counts', 'bin_edges'], histogram):
            assert np.allclose(report['column_summary'][col]['histogram'][key],
                               actual)
...
def test_kde(report, ls):
    ...
    for col in num_cols:
        ...
        for key, actual in zip(['x', 'y'], kde):
            assert np.allclose(report['column_summary'][col]['kde'][key],
                               actual)
...
def test_pair_details_pairdensity(report, ls, col1, col2):
    ...
    for col, key in zip([col1, col2], ['x', 'y']):
        if col in report['column_summary'].keys():
            ...
            assert np.allclose(report['column_summary'][col]['min'],
                               np.min(details['pairdensity'][key]))
            assert (np.max(details['pairdensity'][key]) <=
                    report['column_summary'][col]['max'])

    details_transposed = ls.pair_details(col2, col1)
    assert np.allclose(details['pairdensity']['density'],
...
def test_pair_details_correlation(report, ls, col1, col2):
    ...
    idx = [report['correlation']['_columns'].index(col)
           for col in [col1, col2]]

    for coeff in ['spearman', 'pearson']:
        assert np.allclose(report['correlation'][coeff][idx[0]][idx[1]],
                           details['correlation'][coeff])
        assert np.allclose(details['correlation'][coeff],
...
def test_correlation_matrix(report, ls, col1, col2):
    ...
    correlation_value = (1 if col1 == col2
                         else (ls.pair_details(col1, col2)
    ...
    for i, first_column in enumerate(columns):
        for j, second_column in enumerate(columns):
            expected = scipy.stats.spearmanr(
                df[first_column], df[second_column]).correlation
        ...
        if k not in d2:
            ...
            if isinstance(d1[k], dict):
                ...
                diffs = diffs + find_diff(
                    list_to_dict(d1[k]), list_to_dict(d2[k]), exclude,
                    new_path, False)
            else:
                a = d1[k]
                b = d2[k]
                if not isinstance(a, float) or not (np.isnan(a) and
                                                    np.isnan(b)):
